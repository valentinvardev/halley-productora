...
import { HERO } from "~/app/_datos/categorias";
...
/**
 * Los clips de portada suben de a uno.
 *
 * Son videos, no fotos: mandar seis a la vez por una conexión de subida
 * hogareña los hace competir entre sí y las barras avanzan todas a paso de
 * tortuga. En fila, cada una termina cuando dice.
 */
...
export function useCargaHero(alCompletar: () => void) {
  ...
  const subir = useCallback(
      ...
      const worker = async () => {
        while (cursor < tareas.length) {
          ...
          try {
            ...
            await guardar.mutateAsync({ s3Key: key, tipo });
    ...
    [firmar, guardar, parche, alCompletar],
  );
